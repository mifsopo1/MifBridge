...
#define MIF_BIND(Name) Map.Add(TEXT(#Name), &H_##Name)
			...
			MIF_BIND(add_socket);
			MIF_BIND(run_retarget);
			MIF_BIND(add_virtual_bone);
			MIF_BIND(remove_virtual_bone);
			MIF_BIND(rename_virtual_bone);
			MIF_BIND(add_anim_curve);
			MIF_BIND(set_anim_curve_keys);
			MIF_BIND(remove_anim_curve);
			MIF_BIND(lighting_build_status);
			MIF_BIND(move_actors_to_level);
			MIF_BIND(list_level_instances);
			...
			MIF_BIND(set_widget_animation_range);
			MIF_BIND(remove_widget_animation);
			MIF_BIND(remove_widget_animation_track);
			...
			MIF_BIND(set_niagara_component_parameter);
...
#undef MIF_BIND
	...
	static bool IsReadOnlyEndpoint(const FString& Endpoint)
	{
		static const TSet<FString> ReadOnly = {
			...
			TEXT("nav_project_point"), TEXT("nav_find_path"),
