...
`v0.9.0` &nbsp;·&nbsp; 🎮 **UE 5.3 + 5.7** &nbsp;·&nbsp; 🎨 **Blender 3.6–5.0** &nbsp;·&nbsp; 🔌 **453 UE endpoints** &nbsp;·&nbsp; 🧱 **155 Blender ops** &nbsp;·&nbsp; 🧰 **625 MCP tools** &nbsp;·&nbsp; 🧪 **180 test suites**
...
| Half | State |
|---|---|
| 🎮 **UE plugin + MCP server** | **Mature.** Endpoint and suite counts are in the version line at the top of this file, which is generated and checked at packaging time rather than typed here. Last recorded sweep (`tools/suite_results.json`, 2026-09-01): **173 suites — 167 passed, 2 failed, 4 skipped, 0 editor deaths**. Both failures are the same known bug — a suite adopting a fixture another suite created, which passes alone and only fails on a second pass — and fixes for it are committed and awaiting the confirming sweep. The 3 suites that drive PIE are excluded from unattended sweeps and are named in the run output rather than counted as passing — starting PIE saturates the game thread, and the bridge runs every endpoint on it. |
| 🎨 **Blender addon** | **Mature.** Modelling, materials, UV and lightmap channels, rigging and weights, lights and shadows, cameras, animation and drivers, geometry‑node, shader and COMPOSITOR authoring, collections and view layers, physics, colour management, ray casts and overlap queries, rendering including an out‑of‑process animation render, and interchange in and out — glTF, OBJ, USD, Alembic, STL, PLY and FBX. The op count is in the badge above, which `make_release` refuses to package against when it is stale; it is not restated here, because this cell said "20 ops" and then "68 ops" and was wrong both times within days. **Almost every op is exercised on 3.6.23, 4.2.17 LTS, 4.4.0 and 5.0.1** by `blender_version_matrix.py`, which is what caught three ops that had never worked on any build. Eight are deliberately not run - the five generator ops reach a network service, `render_animation` spawns a second Blender, `bake_texture` needs a fixture no op here can build, and `run_python` executes whatever it is given. The tool prints that list every run rather than rounding it up to "every". |

> **One external dependency, and only one.** The five `gen_*` ops (text/image → 3D) drive a **ComfyUI** instance you run yourself — MifBridge does not ship or install it, and no models or weights are in the release. Default `127.0.0.1:8188`, overridable with `MIF_COMFY_HOST`. Without it those five refuse with a message naming the command to start one; **every other Blender op and every UE endpoint is unaffected.**
...
| For | You need |
|---|---|
| 🎮 **UE plugin** | **Unreal Engine 5.3 or 5.7** — built and tested continuously against 5.3.2; against a stock 5.7 it COMPILES AND LINKS at the current 453-endpoint surface, rechecked 2026-09-03 and recorded in `tools/engine_probe_result.json` against the exact Source commit; and as of 2026-09-05 it has been RUN there too - loaded into a stock 5.7 editor on a real uncooked game project of 35,725 assets, reporting healthy with all 453 endpoints registered and zero policy contradictions, with the full test-suite set swept against it. Those are different claims and both now hold; before that date only the first did. Editor‑only C++, so it must be built against the engine you actually run; a marketplace prebuilt will not ABI‑match a source build. Win64. |
...
**Some suites also run with no backend at all**, which is what you have when somebody else is using
the editor:

```bash
python tools/run_all_suites.py --offline --once   # launches nothing, names what it ran
```

Seconds, no editor and no Blender — 190 assertions across eight suites when it was measured
(2026-09-03), and the command reports the current figures rather than this page doing it.
`test_payload_contract` is the one to know about: it stubs both transports and pins the rule that
`_post` and `_blender` drop `None` and **send every other falsy value** — the contract six shipped
bugs turned on, each of which was reachable without a machine.
