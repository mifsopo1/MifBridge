...
- [ ] **create_asset carries 11 asset types whose validating creators were never built** (hours)
      ...
      SO THE REAL QUESTION IS NARROWER: of the eleven types, which are in neither tier? None of
      Curve, CurveTable, StringTable, MaterialParameterCollection, RuntimeVirtualTexture,
      InputAction, InputMappingContext, NiagaraSystem, SoundCue or MetaSoundSource appears in
      FactoryInitClasses - but that list was GENERATED by audit_factory_init.py, so their absence
      may correctly mean their factories add nothing. Run that tool before assuming a gap; it is the
      instrument that decides this, and it is already written.
