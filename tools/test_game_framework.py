...
def main():
    ...
    # THE WORST OF THE THREE THIS DETECTOR FOUND, and not by a subtle margin: there is a literal
    # `return 3` seven lines below, between the start_pie and the stop_pie at the bottom. On a PIE
    # session that fails to reach state=running this suite returned and left the editor playing.
    # An uncaught TypeError further down did the same on the ordinary case of a spawn failing.
    try:
        ...
        # Unconditional, and read back rather than assumed: stop_pie returning ok proves the
        # request was accepted, not that the session ended.
        M.raw_post("stop_pie", {})
