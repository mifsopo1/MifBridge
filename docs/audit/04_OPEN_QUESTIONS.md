...
- FMigrationOptions prompting — **READ 2026-08-31: a dialog-free migrate IS promisable, on the THREE-argument overload.** `FMigrationOptions::bPrompt` defaults to **false** in the constructor on both 5.3 (IAssetTools.h:227) and 5.7 (:262), and its own comment says "Prompt user for confirmation (always false through scripting)". `AssetConflict` defaults to `EAssetMigrationConflict::Skip`. WHICH OVERLOAD MATTERS: `MigratePackages(PackageNames)` at :516 is the interactive one; `MigratePackages(PackageNames, DestinationPath, Options)` at :520 is the one that takes the struct. Note `bPrompt` is a BARE bool with no UPROPERTY while `AssetConflict` and `OrphanFolder` have `BlueprintReadWrite` - so the conflict policy is reachable reflectively but bPrompt is not, which does not matter given it already defaults false. Unrelated but adjacent, so nobody reaches for it by mistake: `ExportAssetsWithDialog` (:573) is exactly what its name says and is not a migrate path.
- BeginAdvancedCopyPackages — **READ 2026-08-31.** Two overloads (IAssetTools.h:529, :532). The second is `UFUNCTION(BlueprintCallable)` and takes an `FAdvancedCopyCompletedEvent& OnCopyComplete` - that IS the async completion shape the entry was asking for; the first is fire-and-forget with no way to know when it finished, which is the wrong one for a bridge call that must report an outcome. Prompting was not settled from the header alone and stays open, but it is tier-3 anyway. **INCIDENTAL FINDING, and it is clean:** `IAssetTools::FixupReferencers` three lines below (:538) defaults `bCheckoutDialogPrompt = true`, and MifBridge already calls it with an explicit `/*bCheckoutDialogPrompt*/ false` (MifBridgeCooked.cpp:1549). Its surrounding comment is worth reading before anyone touches this family: `GIsRunningUnattendedScript` suppresses `FMessageDialog` but NOT `SDiscoveringAssetsDialog`, which is a raw Slate window, so `IAssetRegistry::IsLoadingAssets()` is checked first - and calling IAssetTools directly rather than the Content Browser path skips two further modals that live in the CALLER. Checked so nobody re-audits it.
...
- Conduit/alias state nodes — **HEADERS READ 2026-08-31: both are SPAWNABLE by the pattern add_anim_state already uses.** Both live in AnimGraph/**Public**/ and are `UCLASS(MinimalAPI)`, which exports the UClass - so `NewObject<UAnimStateConduitNode>(Graph)` and `StaticClass()` work cross-module even though member functions are not exported. That is enough, because `PlaceAndInit` only calls virtual `UEdGraphNode` methods (AddNode, PostPlacedNewNode) dispatched through the vtable. `UAnimStateAliasNode::GetAliasedStates()` IS `ANIMGRAPH_API` (AnimStateAliasNode.h:61-62), so the alias-specific wiring is reachable too, and its `bGlobalAlias` is `UPROPERTY(EditAnywhere, BlueprintReadWrite)`. THE HAZARD, and it is the same one add_anim_state already documents at MifBridgeAnimation.cpp:1797: `FAnimStateNodeNameValidator` does a `CastChecked` on the node's OUTER, which is FATAL rather than an error - so the node must be created into the state machine's own graph, not merely added to it afterwards. Same class of ordering trap as UK2Node_MathExpression above.
