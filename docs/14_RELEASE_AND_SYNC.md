...
| Engine | Status | Notes |
|---|---|---|
| 5.3.2 | **built and tested** | the cooked DDS2 SDK — the primary target. As of 2026-08-30: 421 endpoints, 144 test suites (42 of them not yet in a full sweep), the last recorded double-pass sweep (`run_all_suites.py`) clean at 204 runs over 102 distinct suites, 0 failures, 0 editor deaths. The "148 runs across 74 suites" figure this row used to carry was from 2026-08-26 - superseded, not corrected in place, because the underlying numbers (endpoint count, suite count) both moved and a stale count is worse than no count. |
